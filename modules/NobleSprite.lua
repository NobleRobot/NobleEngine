...
function NobleSprite:play() self:setUpdatesEnabled(true) end

--- This will disable the update loop for this NobleSprite, which also causes its Noble.Animation to pause.
function NobleSprite:pause() self:setUpdatesEnabled(false) end
...
function NobleSprite:stop()
  ...
  if self.animation ~= nil then self.animation.currentFrame = self.animation.current.startFrame end
end
...
function NobleSprite:superAdd() NobleSprite.super.add(self) end
...
function NobleSprite:superRemove() NobleSprite.super.remove(self) end
