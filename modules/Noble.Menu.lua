...
function Noble.Menu.new(
  ...
  __margin,
  __font,
  __selectedCornerRadius,
  __selectedOutlineThickness
)
  ...
  function menu:addItem(__nameOrKey, __clickHandler, __position, __displayName, __displayNameIsALocalizationKey)
    ...
    if __position ~= nil then
      ...
      for key, value in pairs(self.itemPositions) do
        if value >= __position then self.itemPositions[key] = self.itemPositions[key] + 1 end
      end
  ...
  function menu:updateWidths(__nameOrKey, __string)
    if __string == nil then __string = __nameOrKey end
  ...
  function menu:removeItem(__menuItem)
    ...
    if __menuItem == nil then __menuItem = #self.itemNames end
    ...
    for key, value in pairs(self.itemPositions) do
      if value > itemPosition then self.itemPositions[key] = self.itemPositions[key] - 1 end
    end
    ...
    if self.currentItemNumber == itemPosition and self.currentItemNumber ~= 1 then
      ...
      if self.isActive() == false then self:setSelectedRow(0) end
    end
  ...
  -- @treturn bool
  function menu:isActive() return active end
  ...
  function menu:select(__menuItem, __force)
    if self:isActive() or __force then
      if type(__menuItem) == "number" then
        if __menuItem < 1 then error "BONK: _menuItem must be a number greater than 0 (or a string)." end
  ...
  function menu:draw(__x, __y)
    ...
    self:drawInRect(
      ...
      __y,
      self.width,
      ((self.textHeight + self.padding + self.margin) * #self.itemNames)
        + (self.selectedOutlineThickness * 2)
        - self.margin
    )
  end
  ...
  function menu:drawSelectedItem(__x, __y, __itemIndex)
    ...
    elseif self.alignment == Noble.Text.ALIGN_RIGHT then
      ...
      xAdjustmentRect = self.width
        - self.itemWidths[self.itemNames[__itemIndex]]
    ...
    Graphics.fillRoundRect(
      ...
      self.itemWidths[self.itemNames[__itemIndex]] + self.horizontalPadding,
      self.textHeight + self.padding,
