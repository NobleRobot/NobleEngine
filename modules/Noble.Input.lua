...
-- @see NobleScene.inputHandler
function Noble.Input.getHandler() return currentHandler end
...
function Noble.Input.setHandler(__inputHandler)
  if currentHandler ~= nil then playdate.inputHandlers.pop() end
...
-- @see setHandler
function Noble.Input.clearHandler() Noble.Input.setHandler() end
...
function Noble.Input.getEnabled() return cachedInputHandler == nil end
...
function Noble.Input.setCrankIndicatorStatus(__active, __evenWhenUndocked)
  if __active then UI.crankIndicator:start() end
...
function Noble.Input.getCrankIndicatorStatus() return crankIndicatorActive, crankIndicatorForced end
...
function Noble.Input.update()
  ...
  if currentHandler.AButtonHold ~= nil then
    if playdate.buttonIsPressed(playdate.kButtonA) then
      if AButtonHoldBufferCount == buttonHoldBufferAmount then
        currentHandler.AButtonHold() -- Execute!
      else
        AButtonHoldBufferCount = AButtonHoldBufferCount + 1
      end -- Wait another frame!
    end
  ...
  if currentHandler.BButtonHold ~= nil then
    if playdate.buttonIsPressed(playdate.kButtonB) then
      if BButtonHoldBufferCount == buttonHoldBufferAmount then
        currentHandler.BButtonHold()
      else
