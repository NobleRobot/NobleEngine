...
function Noble.new(StartingScene, __transitionDuration, __transitionType, __enableDebugBonkChecking)
  ...
  if __transitionDuration ~= nil then transitionType = __transitionType or Noble.TransitionType.CROSS_DISSOLVE end
...
-- @treturn bool
-- @see Noble.Bonk
function Noble.engineInitialized() return engineInitialized end
...
function Noble.transition(NewScene, __duration, __transitionType, __holdDuration)
  ...
  if Utilities.startsWith(currentTransitionType, Noble.TransitionType.DIP) then
    ...
    onMidpoint()
    transitionSequence = Sequence.new():from(0):to(1, duration, Ease.linear):callback(onComplete)
  end
...
local function transitionUpdate()
  ...
  elseif currentTransitionType == Noble.TransitionType.CROSS_DISSOLVE then
    ...
    -- end
    if progress < 1 then
      previousSceneScreenCapture:drawFaded(
        0,
        0,
        1 - Ease.inOutQuart(progress, 0, 1, 1),
        Graphics.image.kDitherTypeBayer4x4
      )
      -- else
  ...
  elseif currentTransitionType == Noble.TransitionType.SLIDE_OFF_LEFT then
    if progress < 1 then previousSceneScreenCapture:draw(Ease.inQuart(progress, 0, 1, 1) * -400, 0) end
  ...
  elseif currentTransitionType == Noble.TransitionType.SLIDE_OFF_UP then
    if progress < 1 then previousSceneScreenCapture:draw(0, Ease.inQuart(progress, 0, 1, 1) * -240) end
  elseif currentTransitionType == Noble.TransitionType.SLIDE_OFF_DOWN then
    if progress < 1 then previousSceneScreenCapture:draw(0, Ease.inQuart(progress, 0, 1, 1) * 240, 0) end
  end
...
-- @treturn NobleScene
function Noble.currentScene() return currentScene end

--- Get the name of the current scene
-- @treturn string
function Noble.currentSceneName() return currentScene.name end
...
function playdate.update()
  ...
  if Noble.showFPS then playdate.drawFPS(4, 4) end
...
function playdate.gameWillPause()
  if currentScene ~= nil then currentScene:pause() end
end

function playdate.gameWillResume()
  if currentScene ~= nil then currentScene:resume() end
end
