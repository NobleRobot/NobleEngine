...
function Sequence.setPreviousUpdateTime(currentTime)
  if currentTime == nil then currentTime = playdate.getCurrentTimeMilliseconds() end
...
function Sequence.print() print("Sequences running:", #_runningSequences) end
...
function Sequence:to(to, duration, easingFunction, ...)
  ...
  if type(easingFunction) == "string" then easingFunction = _easings[easingFunction] or _easing.inOutQuad end
...
function Sequence:get(time)
  ...
  if self.cachedResultTimestamp == time then return self.cachedResult end

  -- we calculate and cache the result
  local clampedTime = self:getClampedTime(time)
  local easing = self:getEasingByTime(clampedTime)
  local result = easing.fn(
    clampedTime - easing.timestamp,
    easing.from,
    easing.to - easing.from,
    easing.duration,
    table.unpack(easing.params or {})
  )
...
function Sequence:updateCallbacks(dt)
  ...
  local callTimeRange = function(clampedStart, clampedEnd)
    ...
    for index, cbObject in pairs(self.callbacks) do
      if cbObject.timestamp >= clampedStart and cbObject.timestamp <= clampedEnd then
        if type(cbObject.fn) == "function" then cbObject.fn() end
      end
    end
  end
  ...
  if dt > self.duration then callTimeRange(0, self.duration) end
...
function Sequence:addRunning()
  if self:isEmpty() or self.isRunning then return end
...
function Sequence:removeRunning()
  ...
  if indexInRunningTable then table.remove(_runningSequences, indexInRunningTable) end
...
function Sequence:isDone() return self.time >= self.duration and not self.loopType end

function Sequence:isEmpty() return self.easingCount == 0 end

-- new easing function
function _easings.flat(t, b, c, d) return b end

math.clamp = math.clamp
  or function(a, min, max)
    if min > max then
      min, max = max, min
    end
