...
function Utilities.isNull(__object) return __object == nil or __object == "" end
function Utilities.isNotNull(__object) return __object ~= nil and __object ~= "" end

function Utilities.startsWith(__string, __start) return __string:sub(1, #__start) == __start end

function Utilities.endsWith(__string, __ending) return __ending == "" or __string:sub(-#__ending) == __ending end

Utilities.varName = {}
setmetatable(Utilities.varName, {
  __index = function(self, k, v) return string.format("%s", k) end,
})

function Utilities.getHypotenuse(__x, __y) return math.sqrt((__x * __x) + (__y * __y)) end

function Utilities.average(__a, __b) return (__a + __b) / 2 end

function Utilities.screenshot() return Graphics.getDisplayImage() end
...
function math.ringInt(a, min, max) return math.ring(a, min, max + 1) end
function math.approach(value, target, step)
  if value == target then return value, true end
...
function math.sign(v) return (v >= 0 and 1) or -1 end
...
function table.filter(t, filterIter)
  ...
  for _, value in pairs(t) do
    ...
    if filterIter(value) then table.insert(out, value) end
  end
